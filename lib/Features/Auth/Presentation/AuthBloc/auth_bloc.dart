...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Future<void> _registerUser(
    ...
    emit(BannerLoading());
    final res = await rep.authenticateUser(
      event.email,
      event.password,
      event.phoneNumber,
      event.userName,
    );
    res.fold((l) => emit(Failure(message: l)), (r) {
      print('object');
      emit(UserRegistered(email: r.email));
    });
  }

  Future<void> _sendVerificationEmail(
    EmailConfirmation event,
    Emitter<AuthState> emit,
  ) async {
    emit(BannerLoading());
    final res = await rep.emailVerification();
    res.fold((l) => emit(Failure(message: l)), (r) => emit(EmailSent()));
  }
  ...
  Future<void> _signInUser(SignInEvent event, Emitter<AuthState> emit) async {
    ...
    res.fold((l) => emit(Failure(message: l)), (r) => emit(SignedIn()));
  }
  ...
  Future<void> _onCheckIfFirstTime(
    CheckIfFirstTimeEvent event,
    Emitter<AuthState> emit,
  ) async {
    emit(BannerLoading());
